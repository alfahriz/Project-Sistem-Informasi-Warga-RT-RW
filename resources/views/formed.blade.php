...
@endif
	<label>NOKK: </label>
	<input name="nokk" type="text" class="form-control"  value="{{ $keluarga->nokk }}" readonly>
	@if($errors->has('nokk'))
...
@endif
	...
	<input name="namakpk" type="text" class="form-control" value="{{ $keluarga->namakpk }}">
...
@endif
	...
	<input name="tl" type="date" class="form-control" value="{{ date('Y-m-d', strtotime($keluarga->tl)) }}">
	@if($errors->has('tl'))
		<span class="help-block">
...
@endif
	...
	<input name="notelefon" type="text" class="form-control" value="{{ !empty($keluarga->telefon->notelefon) ? $keluarga->telefon->notelefon: '' }}">
...
@endif
	...
	<select name="idstating" id="idstating" class="form-control">
		...
		@foreach ($statinglist as $idstating => $name)
			<option value="{{ $idstating }}" @if ($idstating==$keluarga->idstating) {{'selected'}} @endif>{{ $name }}</option>
		@endforeach
	</select>
...
@endif
	...
	<select name="idalamat" id="idalamat" class="form-control">
		...
		@foreach ($alamatlist as $idalamat => $name)
			<option value="{{ $idalamat }}" @if ($idalamat==$keluarga->idalamat) {{'selected'}} @endif>{{ $name }}</option>
		@endforeach
	</select>
...
@endif
	...
	<input name="norumah" type="text" class="form-control" value="{{ $keluarga->norumah }}">
...
@endif
	...
	@if(count($bansoslist)>0)
		...
		@foreach ($bansoslist as $idbansos => $name)
			<label>
				<input type="checkbox" name="bansos[]" id="bansos" value="{{ $idbansos }}" @foreach ($keluarga->bansos as $bankel) {{ ($bankel->idbansos==$idbansos) ? 'checked' : ''}} @endforeach> {{ $name }}
			</label>
		@endforeach
		</div>
	@else
	<p>Tidak Ada Bantuan Sosial.</p>
	@endif
